...
sub perform {
  ...
  for my $sampleName ( sort keys %mgffiles ) {
    ...
    foreach (@isotopes) {
        ...
        my $resultName = substr(basename($samples),0,-4)."_msgfplus"."_".$_.".mzid";    
...
echo msgfplus_start=`date` 

java -Xmx30g -jar $executable -s $samples -d $proteindb -o $resultName -mod $cfgfile -thread $threadcount $option
